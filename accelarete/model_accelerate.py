...
class MTLModels(nn.Module):
    def __init__(self, transformer, drop_out, heads, data_dict, model_name, num_efl, num_dfl):
        ...
        # # self.task_ident_vector = torch.tensor([0] * len(self.heads), dtype=torch.long)
        # self.task_ident_vector = torch.tensor([0] * len(self.heads), dtype=torch.long)
        # print('@'*50)
        # print(self.task_ident_vector.get_device())
    ...
    def forward(self, iputs, head):
        ...
        #     self.task_ident_vector.resize_(len(self.heads),out.shape[0])
        #     self.task_ident_vector[:] = 0
        #     self.task_ident_vector[:, self.heads.index(head)] = 1
        #     # self.task_ident_vector = self.task_ident_vector.expand(out.shape[0], -1)
        #     print('#'*50)
        #     print(self.task_ident_vector.get_device())
        #     print(out.get_device())
            
        #     out = torch.cat((out,self.task_ident_vector), 1)
