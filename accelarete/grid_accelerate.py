...
import dataset
# import engine
import engine_accelarate as engine
import torch
import pandas as pd
import numpy as np
import math
...
from model_accelerate import MTLModels
...
class CrossValidation(MetricTools, StatisticalTools):
    ...
    def run(self):
        ...
        model, optimizer, train_data_loader, scheduler = accelerator.prepare(model, optimizer, train_data_loader, scheduler)
        ...
        for epoch in range(1, config.EPOCHS+1):
            output_train = engine.train_fn(train_data_loader, model, optimizer, device, scheduler, self.heads, accelerator)
